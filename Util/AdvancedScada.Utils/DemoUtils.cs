...
public class DemoUtils
{
    ...
    public static string  ReadResultRender<T>(OperateResult<T> result,  string address,ref object b)
    {
        if (result.IsSuccess)
        {
                b= result.Content;
                return DateTime.Now.ToString("[HH:mm:ss] ") + $"[{address}] {result.Content}{Environment.NewLine}";
        }
        else
        {
