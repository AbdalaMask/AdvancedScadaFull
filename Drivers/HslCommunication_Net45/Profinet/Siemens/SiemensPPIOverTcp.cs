...
{
    /// <summary>
    /// 西门子的PPI协议，在Tcp上的实现，适用于s7-200plc，注意，本类库有个致命的风险需要注意，由于本类库的每次通讯分成2次操作，故而不支持多线程同时读写，当发生线程竞争的时候，会导致数据异常，
    ...
    /// 注意：M地址范围有限 0-31地址
    /// </remarks>
    public class SiemensPPIOverTcp : NetworkDeviceSoloBase<ReverseBytesTransform>
    {
        #region Constructor
        ...
        public override OperateResult<byte[]> Read(string address, ushort length)
        {
            ...
            // 第二次数据交互
            OperateResult<byte[]> read2 = ReadFromCoreServer(executeConfirm);
