    ...
    public class RedisClient : NetworkDoubleBase<HslMessage, RegularByteTransform>
    {
        ...
        protected override OperateResult InitializationOnConnect(Socket socket)
        {
            if (!string.IsNullOrEmpty(this.password))
            {
                ...
                if (dbBlock > 0)
                {
                    command = RedisHelper.PackStringCommand(new string[] { "SELECT", dbBlock.ToString() });

                    read = ReadFromCoreServer(socket, command);
                    if (!read.IsSuccess) return OperateResult.CreateFailedResult<string>(read);

                    msg = Encoding.UTF8.GetString(read.Content);
                    if (!msg.StartsWith("+")) return new OperateResult<string>(msg);
                }
