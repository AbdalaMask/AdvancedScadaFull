...
{
    public partial class SQLFormCreate : KryptonForm
    {
        ...
        private void SQLFormCreate_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (instances.Length > 0)
                    foreach (var element in instances)
                        if (element == "MSSQLSERVER")
                            ComServerName.Items.Add(Environment.MachineName);
                        else
                            ComServerName.Items.Add(Environment.MachineName + "\\" + element);
                ComFiledType.Items.AddRange(Enum.GetNames(typeof(SqlDbType)));
            }
            catch (Exception ex)
            {
